<template>
  <div>
      ...
      <el-col :span="4" :xs="{ span: 10 }" :sm="{ span: 8 }" type="flex" align="right" class="header-user">
        <el-popover popper-class="head_popper" placement="bottom" trigger="hover">
          <div class="user-tip">
            ...
            <p @click="pointsDialogFormVisible = true">电量管理</p>
    ...
    <!-- 电量管理 -->
    <el-dialog title="电量管理" :visible.sync="pointsDialogFormVisible" @open="getPointsInfo()" width="500px">
      <div style="margin: -20px 0px 50px 0px;color:black">用电量余额：{{ this.points }}</div>
      <div class="user-dialog-footer">
        <el-button size="medium" type="primary" v-if="!isModify" @click="castingPoints()">铸造电量</el-button>
        <el-button size="medium" type="primary" v-if="!isModify" @click="estructionPoints()">销毁电量</el-button>
        <el-button size="medium" type="primary" v-if="!isModify" @click="queryPoints()">查询电量</el-button>
        <el-button size="medium" type="primary" v-if="isModify" @click="changeUserInfo('ruleUserForm')">确认修改</el-button>
  ...
  methods: {
    ...
    async queryPoints() {
      this.$prompt('请输入密码', '提示', {
        ...
        this.$alert("余额："+res.data.amount, '电量中心数据', {
          confirmButtonText: '确定',
